...
export async function GET(request: NextRequest, { params }: { params: { filePath: string } }) {
  ...
  try {
    ...
    // Security check: resolve the path and verify it's still under an allowed root.
    const isAllowed = !pathContainsTraversal(decodedFilePath) && (await isPathAllowed(resolvedFilePath, allowedDirs));
